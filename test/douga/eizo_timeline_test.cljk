...
(deftest render-plan-rejects-unsupported-video-track-transitions
  (testing ":slide (and other non-:dissolve/:wipe types) still have no ffmpeg render path"
    (let [timeline (build-timeline)
          video-track (first (:timeline/tracks timeline))
          [c0 c1] (:track/clips video-track)
          tr (tl/transition {:id :t1 :type :slide
...
(defn- video-track-with-transition
  "Shared fixture for the :dissolve/:wipe acceptance tests below: scene1
  (72 frames, timeline-start 96) overlaps scene0's tail by 12 frames per
  kami.eizo.timeline's overlap semantics: move scene1 back so
  clip-end(scene0)=96 == timeline-start(scene1)+duration(12), then bridge
  the two clips with a transition of the given type."
  [timeline transition-type]
  (let [video-track (first (:timeline/tracks timeline))
        ...
        tr (tl/transition {:id :t1 :type transition-type
                            :from-clip (:clip/id c0) :to-clip (:clip/id c1)
                            :duration 12})]
    (assoc video-track :track/transitions [tr])))
...
(deftest render-plan-accepts-dissolve-video-track-transitions
  (testing ":dissolve now has a real render path (douga.ffmpeg/xfade-transition-cmd) -- no longer rejected"
    (let [timeline (build-timeline)
          video-track (video-track-with-transition timeline :dissolve)
          timeline (assoc-in timeline [:timeline/tracks 0] video-track)
...
(deftest render-plan-accepts-wipe-video-track-transitions
  (testing ":wipe has a real render path (douga.ffmpeg/xfade-transition-cmd, wipeleft mode) -- no longer rejected"
    (let [timeline (build-timeline)
          video-track (video-track-with-transition timeline :wipe)
          timeline (assoc-in timeline [:timeline/tracks 0] video-track)
          validation (tl/validate-timeline timeline)]
      (is (:valid? validation) (pr-str (:errors validation)))
      (let [plan (det/render-plan timeline)]
